...
import { transferWinners } from "./action";
...
export function TransferWinnersForm({ rounds, raceId }: TransferAllFormProps) {
  const { replace } = useRouter();
  const formMethods = useForm<z.infer<typeof TransferWinnersSchema>>({
    resolver: zodResolver(TransferWinnersSchema),
    defaultValues: {
      raceId,
      fromRoundIndex: 0,
      toRoundIndex: 0,
  ...
  const handleSubmit = formMethods.handleSubmit(async (data) => {
    const result = await transferWinners(data);
