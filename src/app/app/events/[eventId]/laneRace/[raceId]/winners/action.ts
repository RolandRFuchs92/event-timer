...
import { TransferWinnersSchema } from "./schema";
import { _db } from "@/lib/db";
import { uniq } from "lodash";
import { revalidatePath } from "next/cache";

export const transferWinners = action(TransferWinnersSchema, async (input) => {
  if (input.fromRoundIndex === input.toRoundIndex)
    throw new Error("You may not transfer from one round and back to itself.");
  ...
  const toRound = race.rounds.find((i) => i.round_index === input.toRoundIndex);
